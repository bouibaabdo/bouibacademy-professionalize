...
import { Calendar, MapPin, Trophy, Radio, ExternalLink, Clock, Tv } from "lucide-react";
import {
  getWorldCupMatches,
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "كأس العالم 2026 — بث مباشر وترتيب المجموعات" },
      {
        ...
        content: "جدول شامل، ترتيب المجموعات، ومباريات اليوم مع القنوات الناقلة وروابط المشاهدة المباشرة.",
      },
      { property: "og:type", content: "website" },
      { property: "og:url", content: "https://www.bouibacademy.me/worldcup" },
    ],
    links: [{ rel: "canonical", href: "https://www.bouibacademy.me/worldcup" }],
  }),
...
function statusOf(m: WCMatch) {
  ...
  const finished =
    ...
    /انتهت|انتهى|finished|full\s*time/i.test(raw);
  const live =
    !finished &&
...
const TEAM_AR_TO_EN: Record<string, string[]> = {
  "السعودية": ["saudi arabia"], "المغرب": ["morocco"], "مصر": ["egypt"],
  "الجزائر": ["algeria"], "تونس": ["tunisia"], "قطر": ["qatar"],
  "الإمارات": ["united arab emirates", "uae"], "الأردن": ["jordan"],
  "العراق": ["iraq"], "إيران": ["iran"], "اليابان": ["japan"],
  "كوريا الجنوبية": ["south korea", "korea republic"], "أستراليا": ["australia"],
  "فرنسا": ["france"], "إنجلترا": ["england"], "إسبانيا": ["spain"],
  "ألمانيا": ["germany"], "إيطاليا": ["italy"], "البرتغال": ["portugal"],
  "هولندا": ["netherlands"], "بلجيكا": ["belgium"], "كرواتيا": ["croatia"],
  "الدنمارك": ["denmark"], "السويد": ["sweden"], "النرويج": ["norway"],
  "سويسرا": ["switzerland"], "بولندا": ["poland"], "صربيا": ["serbia"],
  "النمسا": ["austria"], "أوكرانيا": ["ukraine"], "اسكتلندا": ["scotland"],
  "ويلز": ["wales"], "البرازيل": ["brazil"], "الأرجنتين": ["argentina"],
  "أوروغواي": ["uruguay"], "الأوروغواي": ["uruguay"], "كولومبيا": ["colombia"],
  "الإكوادور": ["ecuador"], "باراغواي": ["paraguay"], "تشيلي": ["chile"],
  "بيرو": ["peru"], "المكسيك": ["mexico"],
  "الولايات المتحدة": ["united states", "usa"], "كندا": ["canada"],
  "كوستاريكا": ["costa rica"], "بنما": ["panama"], "هندوراس": ["honduras"],
  "السنغال": ["senegal"], "الكاميرون": ["cameroon"], "غانا": ["ghana"],
  "نيجيريا": ["nigeria"], "كوت ديفوار": ["ivory coast", "côte d'ivoire"],
    ...
    <div className="rounded-2xl border border-border bg-card p-5 shadow-card hover:shadow-elegant transition-all">
      ...
      <div dir="ltr" className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
        <div className="flex flex-col items-center text-center gap-2">
          {m.homeBadge ? (
            <img src={m.homeBadge} alt={m.home} loading="lazy" className="h-14 w-14 object-contain" />
          ) : (
            <div className="h-14 w-14 rounded-full bg-surface-muted" />
          )}
          <span dir="rtl" className="font-semibold text-sm">{m.home}</span>
        </div>
        ...
        <div className="flex flex-col items-center text-center gap-2">
          {m.awayBadge ? (
            <img src={m.awayBadge} alt={m.away} loading="lazy" className="h-14 w-14 object-contain" />
          ) : (
            <div className="h-14 w-14 rounded-full bg-surface-muted" />
          )}
          <span dir="rtl" className="font-semibold text-sm">{m.away}</span>
        </div>
      </div>
      ...
      {(s.live || s.upcoming) && (
        <div className="mt-4 pt-4 border-t border-border space-y-2">
          ...
          >
            ...
            {s.live ? "افتح البث المباشر الآن" : "افتح البث المباشر"}
          </a>
          <a
  ...
  return (
    ...
    >
      ...
      <div dir="ltr" className="relative grid grid-cols-[1fr_auto_1fr] items-center gap-3">
        <div className="flex flex-col items-center text-center gap-2">
          {m.homeBadge ? (
            <img src={m.homeBadge} alt={m.home} loading="lazy" className="h-16 w-16 object-contain drop-shadow-sm" />
          ) : (
            <div className="h-16 w-16 rounded-full bg-surface-muted" />
          )}
          <span dir="rtl" className="font-semibold text-sm leading-tight">{m.home}</span>
        </div>
        <div className="text-center min-w-[84px]">
          {m.finished || m.live ? (
            ...
            <div className="inline-flex flex-col items-center">
              ...
              <div dir="rtl" className="mt-1 text-[10px] text-muted-foreground">{MATCH_TIMEZONE_LABEL}</div>
            </div>
          )}
        </div>
        <div className="flex flex-col items-center text-center gap-2">
          {m.awayBadge ? (
            <img src={m.awayBadge} alt={m.away} loading="lazy" className="h-16 w-16 object-contain drop-shadow-sm" />
          ) : (
            <div className="h-16 w-16 rounded-full bg-surface-muted" />
          )}
          <span dir="rtl" className="font-semibold text-sm leading-tight">{m.away}</span>
        </div>
      </div>
        ...
        <div className="relative mt-4 pt-4 border-t border-border grid grid-cols-2 gap-2 text-xs">
          {m.channel && (
            <div className="flex items-center gap-1.5 rounded-lg bg-surface-muted/60 px-2.5 py-2">
              ...
              <span className="font-semibold text-foreground truncate" title={m.channel}>{m.channel}</span>
            </div>
          )}
          {m.commentator && (
            <div className="flex items-center gap-1.5 rounded-lg bg-surface-muted/60 px-2.5 py-2">
              <Radio className="h-3.5 w-3.5 text-primary shrink-0" />
              <span className="text-muted-foreground truncate" title={m.commentator}>{m.commentator}</span>
            </div>
          )}
        </div>
      )}
      ...
      {!m.finished && (
        <div className="relative mt-4 space-y-2">
          ...
          >
            <ExternalLink className="h-4 w-4" />
            {m.live ? "افتح البث المباشر الآن" : "افتح البث المباشر"}
          </a>
          <p className="text-center text-[11px] text-muted-foreground">
            يفتح البث في صفحة مستقلة لتجنب مشاكل التضمين.
          </p>
        </div>
      )}
    </div>
...
function StandingsTable({ standings }: { standings: WCStanding[] }) {
  ...
  if (standings.length === 0) {
    return <div className="text-center py-16 text-muted-foreground">جدول الترتيب سيظهر بعد انطلاق البطولة.</div>;
  }

  return (
    <div className="grid gap-6 md:grid-cols-2">
      {groups.map(([group, rows]) => (
        <div key={group} className="rounded-2xl border border-border bg-card overflow-hidden shadow-card">
            ...
            <table className="w-full text-sm">
              <thead className="bg-surface-muted/50 text-xs text-muted-foreground">
                <tr>
                  ...
                  <th className="px-2 py-2 font-semibold" title="لعب">ل</th>
                  <th className="px-2 py-2 font-semibold" title="فاز">ف</th>
                  <th className="px-2 py-2 font-semibold" title="تعادل">ت</th>
                  <th className="px-2 py-2 font-semibold" title="خسر">خ</th>
                  <th className="px-2 py-2 font-semibold" title="فارق الأهداف">+/-</th>
                ...
                {rows.map((r) => (
                  <tr key={r.team} className="border-t border-border hover:bg-accent/30 transition">
                    ...
                    <td className="px-3 py-2.5">
                      <div className="flex items-center gap-2">
                        {r.badge && <img src={r.badge} alt="" className="h-5 w-5 object-contain" loading="lazy" />}
                        <span className="font-semibold">{r.team}</span>
                      </div>
                    </td>
                    <td className="text-center tabular-nums">{r.played}</td>
                    <td className="text-center tabular-nums text-emerald-600 font-semibold">{r.win}</td>
                    ...
                    <td className="text-center tabular-nums font-extrabold text-primary">{r.points}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function WorldCupPage() {
  ...
  const todayMatches = useMemo(() => data.matches.filter((m) => m.date === todayStr), [data.matches, todayStr]);
      ...
      <section className="relative hero-bg overflow-hidden border-b border-border">
        ...
        <div className="container-page relative py-14 md:py-20 text-center">
          ...
          <p className="mt-5 mx-auto max-w-2xl text-muted-foreground">
            جميع مباريات المونديال بتوقيت المغرب، ترتيب المجموعات لحظة بلحظة، ومباريات اليوم مع القنوات الناقلة وروابط المشاهدة الرسمية المباشرة.
          </p>
          ...
          <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-3 max-w-3xl mx-auto">
            {[
              ...
              <div key={s.l} className="rounded-xl border border-border bg-card px-4 py-3 shadow-soft">
                <div className={`font-display text-2xl font-extrabold tabular-nums ${s.c}`}>{s.v}</div>
            ...
            <div className="mt-6 inline-flex items-center gap-3 rounded-2xl border border-border bg-card px-6 py-4 shadow-card">
              {[
                ...
                <div key={i} className="text-center min-w-[54px]">
                  ...
                  <div className="text-[10px] text-muted-foreground uppercase tracking-wider mt-1">{x.l}</div>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="relative mx-auto max-w-3xl mb-8">
          ...
          <div className="rounded-2xl border border-border bg-card p-3 md:p-4 shadow-card">
            ...
            <div className="mt-3 grid grid-cols-3 gap-2 md:gap-3">
              {tabs.slice(0, 3).reverse().map((t, idx) => {
                const palette = [
                  "bg-[hsl(210_75%_35%)]",
                  "bg-[hsl(140_70%_28%)]",
                  "bg-[hsl(25_85%_42%)]",
                ...
                return (
                  <button
                    ...
                    className={`rounded-xl px-3 py-3 text-sm md:text-base font-bold text-white transition-all shadow-soft ${palette} ${
                      isActive ? "ring-2 ring-offset-2 ring-offset-card ring-white/50 scale-[1.02]" : "opacity-90 hover:opacity-100"
                    }`}
                  >
                    {t.label}
                    {t.count != null && (
              ...
              <div className="mt-3 flex flex-wrap gap-2 justify-center">
                {tabs.slice(3).map((t) => (
                  ...
                  >
                    {t.label}
                    {t.count != null && (
                      <span className={`text-[11px] rounded-full px-2 py-0.5 ${tab === t.key ? "bg-primary-foreground/20" : "bg-accent"}`}>
                        {t.count}
                      </span>
                    )}
                  </button>
          ...
          <div className="space-y-12">
            <div className="text-center mb-2">
              ...
              <p className="mt-2 text-sm text-muted-foreground max-w-2xl mx-auto">
                جدول شامل لمباريات الأمس واليوم والغد مصدره <span className="font-semibold text-foreground">يلا كورة</span>،
                مع القناة الرسمية والمعلّق. زر المشاهدة يفتح صفحة البث المباشر المستقلة بدون تضمين داخل المدونة.
              </p>
            </div>

            {!ykReconciled ? (
              <div className="text-center py-16 text-muted-foreground">جارٍ تحميل مباريات اليوم…</div>
            ) : (
              <>
                {ykReconciled.today.length === 0 && ykReconciled.tomorrow.length === 0 && ykReconciled.yesterday.length === 0 && (
                  <div className="text-center py-16 text-muted-foreground rounded-2xl border border-dashed border-border space-y-2">
                    <p>لا توجد مباريات مُدرجة على <span className="font-semibold text-foreground">يلا كورة</span> لهذه الفترة.</p>
                    <p className="text-xs">يمكنك تصفح المباريات من التبويبات الأخرى (البث المباشر / كأس العالم) أو المحاولة لاحقًا.</p>
                  </div>
                )}
                        ...
                        <div className="relative">
                          ...
                          <div className="relative flex items-center gap-3 mb-5">
                            <span className="inline-flex items-center gap-2 rounded-full bg-red-600 text-white px-3 py-1 text-xs font-extrabold shadow-lg shadow-red-500/30">
                              <span className="h-2 w-2 rounded-full bg-white animate-pulse" /> مباشر الآن
                            </span>
                            <h3 className="font-display font-extrabold text-xl">{liveNow.length} مباراة تُبثّ حالياً</h3>
                          </div>
                          <div className="relative grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                            {liveNow.map((m) => (
                              <YKMatchCard key={m.id} m={m} featured />
                            ))}
                          </div>
                        </div>
                      )}
                      ...
                      {upcomingToday.length > 0 && (
                        <div>
                          <h3 className="font-display font-bold text-lg text-primary mb-4 flex items-center gap-2">
                            <Clock className="h-5 w-5" /> اليوم · {upcomingToday.length} مباراة قادمة
                          </h3>
                          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                            {upcomingToday.map((m) => (
                              <YKMatchCard key={m.id} m={m} />
                            ))}
                          </div>
                        </div>
                      )}
                ...
                {ykReconciled.tomorrow.length > 0 && (
                  <div>
                    <h3 className="font-display font-bold text-lg text-primary mb-4 flex items-center gap-2">
                      <Calendar className="h-5 w-5" /> الغد · {ykReconciled.tomorrow.length} مباراة
                    </h3>
                    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        ...
        {tab === "today" && (
          <div>
            <div className="text-center mb-6">
              <h2 className="font-display text-2xl md:text-3xl font-bold">مباريات اليوم مع القنوات الناقلة</h2>
            ...
            {todayMatches.length === 0 ? (
              ...
              <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                {todayMatches.map((m) => (
                  <MatchCard key={m.id} m={m} favIds={favIds} />
                ))}
              </div>
            )}
          </div>
            ...
            {byDay.map(([day, matches]) => (
              <div key={day}>
                <div className="sticky top-16 z-10 mb-4 -mx-4 px-4 py-2 backdrop-blur bg-background/80 border-b border-border">
                  <h3 className="font-display font-bold text-lg text-primary">{formatDayAr(day)}</h3>
        ...
        {tab === "channels" && (
          <div>
            <div className="text-center mb-8">
              <h2 className="font-display text-2xl md:text-3xl font-bold">قنوات البث الرسمية للمونديال</h2>
              <p className="mt-2 text-sm text-muted-foreground max-w-2xl mx-auto">
                اختر المنصة المتاحة في بلدك لمشاهدة المباريات بجودة عالية وبشكل قانوني — روابط مباشرة إلى صفحات البث.
              </p>
            </div>
              ...
              {OFFICIAL_STREAMS.map((s) => (
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    <div className={`inline-flex h-12 w-12 items-center justify-center rounded-xl text-white ${s.color}`}>
