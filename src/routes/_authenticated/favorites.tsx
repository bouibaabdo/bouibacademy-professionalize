...
import { Bell, BellOff, Mail, MonitorSmartphone, Trash2, Trophy, Calendar, MapPin, CheckCircle2 } from "lucide-react";
...
export const Route = createFileRoute("/_authenticated/favorites")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "أدر مبارياتك المفضلة وقم بتفعيل تنبيهات البريد وإشعارات المتصفح." },
...
interface Fav {
  id: string;
  match_id: string;
  match_data: { home: string; away: string; homeBadge?: string; awayBadge?: string; venue?: string; round?: string };
  kickoff_at: string;
...
function FavoritesPage() {
  ...
  const del = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["favorites"] }); toast.success("حذفت"); },
  });
  const upd = useMutation({
    mutationFn: (v: { id: string; notify_email?: boolean; notify_browser?: boolean }) => update({ data: v }),
      ...
      <main className="container-page py-10">
        ...
        <div className="rounded-2xl border border-border bg-card p-5 shadow-card mb-8">
          <div className="flex items-start gap-4">
            <div className={`h-10 w-10 rounded-xl inline-flex items-center justify-center ${perm === "granted" ? "bg-emerald-100 text-emerald-600" : "bg-primary/10 text-primary"}`}>
              {perm === "granted" ? <CheckCircle2 className="h-5 w-5" /> : <Bell className="h-5 w-5" />}
            </div>
            <div className="flex-1">
              <h3 className="font-semibold">إشعارات المتصفح</h3>
              <p className="text-sm text-muted-foreground mt-1">
                {perm === "granted" && "مفعّلة — ستظهر لك التنبيهات عندما يكون الموقع مفتوحًا في تبويب."}
            ...
            {perm === "default" && (
              <button onClick={requestPerm} className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:opacity-90">
                تفعيل
              </button>
            )}
          </div>
        ...
        {isLoading && <div className="text-center text-muted-foreground py-10">جاري التحميل...</div>}
        ...
        {!isLoading && favs.length === 0 && (
          <div className="rounded-2xl border border-dashed border-border p-10 text-center">
            ...
            <p className="text-muted-foreground">
              ...
              <a href="/worldcup" className="text-primary font-semibold hover:underline">صفحة كأس العالم</a>{" "}
              واضغط ⭐ على أي مباراة.
            </p>
          </div>
        )}
            ...
            return (
              <div key={f.id} className="rounded-2xl border border-border bg-card p-5 shadow-card">
                <div className="flex items-center justify-between text-xs text-muted-foreground mb-3">
                  <span className="inline-flex items-center gap-1.5"><Trophy className="h-3.5 w-3.5 text-primary" />{f.match_data.round ?? "مباراة"}</span>
                  {past && <span className="text-emerald-600 font-semibold">انتهت</span>}
                </div>
                <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 mb-4">
                  <div className="text-center">
                    {f.match_data.homeBadge && <img src={f.match_data.homeBadge} alt={f.match_data.home} className="h-12 w-12 mx-auto object-contain" />}
                  ...
                  <div className="text-center">
                    {f.match_data.awayBadge && <img src={f.match_data.awayBadge} alt={f.match_data.away} className="h-12 w-12 mx-auto object-contain" />}
                    <div className="mt-1 text-sm font-semibold">{f.match_data.away}</div>
                  </div>
                </div>
                <div className="text-xs text-muted-foreground space-y-1 mb-4">
                  <div className="flex items-center gap-1.5"><Calendar className="h-3.5 w-3.5" />{fmt}</div>
                  {f.match_data.venue && <div className="flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5" />{f.match_data.venue}</div>}
                </div>
                <div className="pt-3 border-t border-border space-y-2">
                  <label className="flex items-center justify-between gap-2 text-sm cursor-pointer">
                    <span className="inline-flex items-center gap-2"><Mail className="h-4 w-4 text-muted-foreground" />تنبيه بريد إلكتروني</span>
                    <input
                      type="checkbox"
                  ...
                  <label className="flex items-center justify-between gap-2 text-sm cursor-pointer">
                    <span className="inline-flex items-center gap-2"><MonitorSmartphone className="h-4 w-4 text-muted-foreground" />إشعار متصفح</span>
                    <input
                      type="checkbox"
                  ...
                  >
                    <Trash2 className="h-4 w-4" />إزالة من المفضلة
                  </button>
                </div>
              </div>
            );
